...
def main(argv):
    # Work to eliminate model_weights paths for both models
    ...
    parser.add_argument("-m", "--model_weights", required=False,
                            default="data/open_nsfw-weights.npy",
...
#            frameId = cap.get(1) #current frame number
            ...
            # easiest way to convert to format is save to disk - other modules require a disk file
            # see if can eliminate disk write (speed processing)
...
# un-remark these lines to show the video prior to any gender / age processing as quickly as possible
...
#         cv2.waitKey(1)
            ...
            if len(detected) > 0:
                ...
                for i, d in enumerate(detected):
                    ...
                    draw_label(image2, (x1, y1), label, rectangle_height)
            
            # advise if frame is SFW or not
            if(Nsfw): 
                size = cv2.getTextSize("NSFW", cv2.FONT_HERSHEY_SIMPLEX, 0.8, 1)[0]
                cv2.rectangle(image2, (1, 20 - size[1]),  (1 + size[0], 20), (255, 0, 0), cv2.FILLED)
                cv2.putText(image2, "NSFW", (1, 19), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 1, lineType = cv2.LINE_AA)  
            else: 
                size = cv2.getTextSize("SFW", cv2.FONT_HERSHEY_SIMPLEX, 0.8, 1)[0]
                cv2.rectangle(image2, (1, 20 - size[1]),  (1 + size[0], 20), (255, 0, 0), cv2.FILLED)
                cv2.putText(image2, "SFW", (1, 19), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 1, lineType = cv2.LINE_AA)  
